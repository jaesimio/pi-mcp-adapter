...
async function openInOrcaBrowser(url: string): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    execFile("orca", ["goto", "--url", url], { timeout: 10000 }, (error) => {
      if (error) {
        reject(new Error(error.message || "orca goto failed"));
      } else {
        resolve();
      }
    });
...
export async function maybeStartUiSession(
  ...
  try {
    ...
    if (uiSuppressed) {
      ...
      if (viewerPref === "orca") {
        try {
          await openInOrcaBrowser(handle.url);
          if (remoteLikely) {
